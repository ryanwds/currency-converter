$(document).ready(function() {
    ...
    const currencies = {
        ...
        "SOS":"Somali Shilling","SRD":"Surinamese Dollar","STD":"São Tomé and Príncipe Dobra",
        ...
        "UYU":"Uruguayan Peso","UZS":"Uzbekistan Som","VEF":"Venezuelan Bolívar Fuerte",
        "VES":"Sovereign Bolivar","VND":"Vietnamese Dong","VUV":"Vanuatu Vatu",
        "WST":"Samoan Tala","XAF":"CFA Franc BEAC","XAG":"Silver (troy ounce)",
        "XAU":"Gold (troy ounce)","XCD":"East Caribbean Dollar","XDR":"Special Drawing Rights",
        "XOF":"CFA Franc BCEAO","XPF":"CFP Franc","YER":"Yemeni Rial",
        "ZAR":"South African Rand","ZMK":"Zambian Kwacha (pre-2013)","ZMW":"Zambian Kwacha",
        "ZWL":"Zimbabwean Dollar"
    };

    function populateCurrencyDropdowns() {
        const fromSelect = $('#currency_from');
        const toSelect = $('#currency_to');
        ...
        if (currency_from && currency_to && amount) {
            $.ajax({
                ...
                success: function(response) {
                    var formattedResult = parseFloat(response.result).toFixed(2);
                    $('#result').html('<strong>Resultado:</strong> ' + formattedResult);
                },
                error: function(xhr, status, error) {
                    $('#result').html('<strong>Erro:</strong> ' + (xhr.responseJSON?.error || 'Erro desconhecido'));
                }
            });
        } else {
            $('#result').text('');
    ...
    $('#amount, #currency_from, #currency_to').on('input change', function() {
        convertCurrency();
    });
});